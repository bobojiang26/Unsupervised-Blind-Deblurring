...
class E_content_codebook(nn.Module):
  def __init__(self, input_dim):
    super(E_content_codebook, self).__init__()
    enc_c = []
    tch = 64
...
class G_concat(nn.Module):
  def __init__(self, output_dim_a, output_dim_b, nz):
    ...
    # generator for input_A(domain A)
    tch = 256+self.nz
    decA1 = []
    for i in range(0, 3):
    ...
    # generator for input_B(domain_B)
    tch = 256+self.nz
    decB1 = []
    for i in range(0, 3):
...
class G_concat_codebook(nn.Module):
  def __init__(self, output_dim_a):
    super(G_concat_codebook, self).__init__()
    tch = 256
    dec_share = []
    dec_share += [INSResBlock(tch, tch)]
    self.dec_share = nn.Sequential(*dec_share)

    # generator for training codebook
    tch = 256
    decC1 = []
    for i in range(0, 3):
      decC1 += [INSResBlock(tch, tch)]
    decC2 = ReLUINSConvTranspose2d(tch, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)
    tch = tch//2
    decC3 = ReLUINSConvTranspose2d(tch, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)
    tch = tch//2
    decC4 = [nn.ConvTranspose2d(tch, output_dim_a, kernel_size=1, stride=1, padding=0)]+[nn.Tanh()]
    self.decC1 = nn.Sequential(*decC1)
    self.decC2 = nn.Sequential(*[decC2])
    self.decC3 = nn.Sequential(*[decC3])
    self.decC4 = nn.Sequential(*decC4)

  def forward(self, x):
    out0 = self.dec_share(x)
    out1 = self.decC1(out0)
    out2 = self.decC2(out1)
    out3 = self.decC3(out2)
    out4 = self.decC4(out3)
    return out4
...
class PerceptualLoss16():
    def __init__(self, loss, gpu=0, p_layer=14):
        ...
        self.criterion = loss
#         conv_3_3_layer = 14
        checkpoint = torch.load('/media/zcb/Data/Unsupervised-Domain-Specific-Deblurring-master/src/vggface_path/VGGFace16.pth')
...
def get_scheduler(optimizer, opts, cur_ep=-1):
  if opts.lr_policy == 'lambda':
    def lambda_rule(ep):
      lr_l = 1.0 - max(0, ep - opts.n_ep_decay) / float(opts.n_ep_ts2 - opts.n_ep_decay + 1)
      return lr_l
